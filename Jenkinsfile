pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = "mahimadod/lms-book-service"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/mahimadod/lms-book-service.git'
            }
        }
        ...
        stage('Deploy') {
            steps {
                ...
                bat """
                    docker rm -f lms-book || exit 0
                    docker run -d --name lms-book --network lms-network -p 8081:8081 ${DOCKER_IMAGE}:${BUILD_NUMBER}
                """
            }
        }
    }
